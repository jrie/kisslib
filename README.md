...
* In some cases - for the "jessie" version, the dialog windows for "import files and folders", "edit ebook details" and "set launcher applciations" are all opened but hidden behind the main window. Please note that when those windows are open, the main application state is set to background and not handle input at this time, so you must move the main window away in order to reach the dialogs
